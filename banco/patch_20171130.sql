...
COMMENT ON COLUMN dbesusgestor.tb_planoprograma.ds_detalhamento IS 'Nome da unidade de trabalho';
COMMENT ON COLUMN dbesusgestor.tb_plano.co_programa_pai IS 'Referência ao programa pai';
...
(
  ...
  co_periodicidade int not null,
  co_unidade integer not null,  --- TODO: Unidade que monitora
  CONSTRAINT tb_meta_pkey PRIMARY KEY (co_meta)
);

COMMENT ON TABLE dbesusgestor.tb_meta IS 'Armazena as metas';
COMMENT ON COLUMN dbesusgestor.tb_programa.ds_sigla IS 'Sigla da meta';
COMMENT ON COLUMN dbesusgestor.tb_programa.co_programa IS 'FK para o programa';
COMMENT ON COLUMN dbesusgestor.tb_programa.ds_meta IS 'Descrição da meta';
COMMENT ON COLUMN dbesusgestor.tb_programa.ds_justificativa IS 'Justificativa da meta';
