...
const indicador = {
  ...
  'IBGEMUN': { desc: 'IBGE Municipio',
        metadata: [
          {colType: 'String', colName: 'Região', resumo: 'Região agregada', tipo: 'geo'},
          {colType: 'String', colName: 'Local', resumo: 'Região desagregada', tipo: 'geo'},
          {colType: 'String', colName: 'codigogeo', resumo: 'Código da unidade', tipo: 'id'},
          {colType: 'Numeric', colName: 'Ano', resumo: 'Ano da ocorrência', tipo: 'ano'},
        ],
  ...
  getValores: (req, res)=>{
    ...
    req.swagger.params.codigos.value.forEach((item)=>{
      if(indicador[item]){
        ...
        meta.push({colType: 'Numeric', colName: indicador[item].desc, resumo: indicador[item].resumo, tipo: 'valor'});
      }
    });
