  ...
  stages {
    ...
    stage('Run Live') {
      steps {
        ...
        string(credentialsId: 'RANCHER_SECRET_KEY', variable: 'SECRET_KEY')]) {
          sleep(13)
          sh """curl -u '$ACCESS_KEY:$SECRET_KEY' -X POST -H 'Accept: application/json' -H 'Content-Type: application/json' -d '{}' 'http://138.197.93.63/v2-beta/projects/1a781/services/1s67/?action=finishupgrade'"""
